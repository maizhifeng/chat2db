...
export async function queryDatabase(connectionId, nlQuery) {
  try {
    const result = await apiRequest(`/query/${connectionId}`, {
      method: 'POST',
...
export async function getTables(connectionId) {
  try {
    const tables = await apiRequest(`/connections/${connectionId}/tables`, {
      headers: {
        'Authorization': `Bearer ${getAuthToken()}`,
      },
    });
